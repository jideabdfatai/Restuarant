    ...
    public class TimeConverter
    {
        ...
        public OpenningAndClosingHoursResponse Converter(OpenningAndClosingHoursReqest list)
        {
            ...
            return new OpenningAndClosingHoursResponse
            {
                ...
                //Sunday = list.Sunday.FirstOrDefault().Value == 0? "Sunday:closed": string.Format("Sunday: {0}", PrintTime(TimeSorteer(list.Sunday, list.Saturday, list.Monday, "Saturday", "Monday"))),
                //Monday = list.Monday.FirstOrDefault().Value ? "Monday:Closed" : string.Format("Monday: {0}", PrintTime(TimeSorteer(list.Monday, list.Sunday, list.Tuesday, "Sunday", "Tuesday"))),
                //Tuesday = list.Tuesday.FirstOrDefault().Value == 0 ? "Tuesday:Closed" : string.Format("Tuesday: {0}", PrintTime(TimeSorteer(list.Tuesday, list.Monday, list.Wednesday, "Monday", "Wednesday"))),
                //Wednesday = list.Wednesday.FirstOrDefault().Value == 0 ? "Wednesday:Closed" : string.Format("Wednesday: {0}", PrintTime(TimeSorteer(list.Wednesday, list.Tuesday, list.Thursday, "Tuesday", "Thursday"))),
                //Thursday = list.Thursday.FirstOrDefault().Value == 0 ? "Thursday:Closed" : string.Format("Thursday: {0}", PrintTime(TimeSorteer(list.Thursday, list.Wednesday, list.Friday, "Wednesday", "Friday"))),
                //Friday = list.Friday.FirstOrDefault().Value == 0 ? "Friday:Closed" : string.Format("Friday: {0}", PrintTime(TimeSorteer(list.Friday, list.Thursday, list.Saturday, "Thursday", "Saturday"))),
                //Saturday = list.Saturday.FirstOrDefault().Value == 0 ? "Saturday:Closed" : string.Format("Saturday: {0}", PrintTime(TimeSorteer(list.Saturday, list.Friday, list.Sunday, "Friday", "Sunday"))),


                Sunday = list.Sunday == null ? "Sunday:closed" : string.Format("Sunday: {0}", PrintTime(TimeSorteer(list.Sunday, list.Saturday, list.Monday, "Saturday", "Monday"))),
                Monday = list.Monday == null ? "Monday:Closed" : string.Format("Monday: {0}", PrintTime(TimeSorteer(list.Monday, list.Sunday, list.Tuesday, "Sunday", "Tuesday"))),
                Tuesday = list.Tuesday  == null ? "Tuesday:Closed" : string.Format("Tuesday: {0}", PrintTime(TimeSorteer(list.Tuesday, list.Monday, list.Wednesday, "Monday", "Wednesday"))),
                Wednesday = list.Wednesday == null ? "Wednesday:Closed" : string.Format("Wednesday: {0}", PrintTime(TimeSorteer(list.Wednesday, list.Tuesday, list.Thursday, "Tuesday", "Thursday"))),
                Thursday = list.Thursday == null ? "Thursday:Closed" : string.Format("Thursday: {0}", PrintTime(TimeSorteer(list.Thursday, list.Wednesday, list.Friday, "Wednesday", "Friday"))),
                Friday = list.Friday == null ? "Friday:Closed" : string.Format("Friday: {0}", PrintTime(TimeSorteer(list.Friday, list.Thursday, list.Saturday, "Thursday", "Saturday"))),
                Saturday = list.Saturday == null ? "Saturday:Closed" : string.Format("Saturday: {0}", PrintTime(TimeSorteer(list.Saturday, list.Friday, list.Sunday, "Friday", "Sunday"))),
            };
        ...
        public List<Time> TimeSorteer(List<OpenHourModel> openHourModel, List<OpenHourModel> previousDay, List<OpenHourModel> nextDay, string previousDayAsWord, string nextDayAsWord)
        {
            ...
            if (openhours.Count >= 1 && closehours.Count >= 1)
            {
                ...
                foreach (var openitemhour in openhours)
                {
                    ...
                    var closeitemhour = closehours.Where(a => a.Value > openitemhour.Value).SingleOrDefault();
                    if (closeitemhour != null)
                    {
                        var time = new Time
                        {
                            OpenTime = UnixTimeStampToShortTimeString(openitemhour.Value),
